...
TODO Graphics is going to be... each pixel is both a color and a lambda function, that can be drag-and-dropped onto pixel/function to find/create pixel/function, in which anything can be built, so kind of like icons are within the functions and can be voxel-like at the same time. Paintable, that if a function is at some set of pixels on screen and any of those pixels are painted, the function is replaced everywhere on screen that it occurs with the same function forkEdited to have its "icon" that color at that place (the screen pixels are a tuple such as 2 * 512 * 512 Numbers/lambdas for 512 x 512 resolution).

TODO will use this graphics system, which is voxels of 2^16 x 2^16 x 2^16 and 15 bit color, in nodes containing int64->int32 functions that map possible voxels to buckets where each bucket contains at most 1 voxel, and one of the nodes, for example, contains a float[4][4] affine transform that maps voxels into a long[512*512] of each (x y) position in a rectangle on screen is such a node. See pic at https://github.com/benrayfield/statvoxel as its already doing 3d rotations and movements etc. Lambdasmiter will somehow generate these longs, maybe as 2 int32s (each double can contain an int32) or something like that. This is a pic of statvoxel (not used by lambdasmiter yet)...
