# lambdasmiter
Pure functional VM (TODO 4 independent realtime interoperable VMs to start with) will derive its own prog lang syntaxes. A number is immutable and is a float64 or a list of numbers. A lambda is a number. Few microseconds bytecode verifier. Like SQL, always halts (smites lag and infinite loops and other halting problem things that plague the software industry). GPU. Music tools. Gaming-low-lag. Screen blit. Fork millions of threads. Generated lambdas + bigdata safely share online at 10% lightspeed.
